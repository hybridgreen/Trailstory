...
def test_retrieve_user(setup):
    ...
    fetched = client.get("/users/me", headers={"Authorization": f"Bearer {at}"})
...
def test_update_user(setup):
    ...
    update = {"firstname": "Robert", "lastname": "Spongey"}

    fetched = client.put(
        "/users", data=update, headers={"Authorization": f"Bearer {at}"}
    )
...
def test_delete_user(setup):
    ...
    response = client.delete("/users/", headers={"Authorization": f"Bearer {at}"})
